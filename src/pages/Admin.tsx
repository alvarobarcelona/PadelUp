...
const Admin = () => {
    ...
    const handleDeletePlayer = async (id: string) => {
        ...
        try {
            ...
            await logActivity('ADMIN_DELETE_USER', id, { deleted_id: id });
    ...
    const handleDeleteMatch = async (id: number) => {
        ...
        try {
            ...
            await logActivity('ADMIN_DELETE_MATCH', id.toString(), {
    ...
    const handleApprovePlayer = async (id: string, username: string) => {
        ...
        if (error) {
            ...
            // LOG ADMIN APPROVE
            await logActivity('ADMIN_APPROVE_USER', id, { username });
    ...
    const handleCleanupMessages = async (days: number) => {
        ...
        try {
            ...
            await logActivity('ADMIN_CLEANUP_MESSAGES', null, { days, deleted_count: count });
    ...
    const handleUpdateMatch = async () => {
        ...
        try {
            ...
            await alert({ title: 'Success', message: 'Match updated successfully!', type: 'success' });
            setEditingMatch(null);
            fetchData();
        } catch (error: any) {
            console.error(error);
            await alert({ title: 'Error', message: 'Error updating match: ' + error.message, type: 'danger' });
        } finally {
            setLoading(false);
        }
    };
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                {paginate(logs
                                    ...
                                    .filter(l => normalizeForSearch(JSON.stringify(l)).includes(normalizeForSearch(logSearch))), logsPage)
                                    .map(log => {
                                ...
                                <PaginationControls
                                    ...
                                    totalItems={logs.filter(l => {
                                        const matchesSearch = !logSearch ||
                                            (l.details && JSON.stringify(l.details).toLowerCase().includes(logSearch.toLowerCase())) ||
                                            (l.action && l.action.toLowerCase().includes(logSearch.toLowerCase())) ||
                                            (l.actor?.username && l.actor.username.toLowerCase().includes(logSearch.toLowerCase()));
                                        const matchesType = selectedActions.includes(l.action as any);
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        type="datetime-local"
                                        className="w-full bg-slate-800 border-slate-700 rounded p-2 text-white"
                                        value={new Date(editingMatch.created_at).toISOString().slice(0, 16)}
                                        onChange={(e) => setEditingMatch({ ...editingMatch, created_at: new Date(e.target.value).toISOString() })}
                                    />
                                </div>
                                <div>
                                    ...
                                    <select
                                        className="w-full bg-slate-800 border-slate-700 rounded p-2 text-white"
                                        value={editingMatch.club_id || ''}
                                        onChange={(e) => setEditingMatch({ ...editingMatch, club_id: e.target.value ? parseInt(e.target.value) : null })}
                                    >
                                ...
                                <div>
                                    <label className="text-xs text-slate-400 block mb-1">Commentary</label>
                                    <textarea
                                        className="w-full bg-slate-800 border-slate-700 rounded p-2 text-white resize-none h-24"
                                        value={editingMatch.commentary || ''}
                                        onChange={(e) => setEditingMatch({ ...editingMatch, commentary: e.target.value })}
                                        placeholder="Add notes about this match..."
                                    />
                                </div>
                                <Button onClick={handleUpdateMatch} className="w-full mt-4 flex items-center justify-center gap-2">
                                    <Save size={18} />
                                    Save Changes
                                </Button>
                            </div>
